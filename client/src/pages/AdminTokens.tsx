...
export default function AdminTokens() {
    ...
    const filteredTokens = useMemo(() => {
        return safeTokens.filter(t => {
            ...
            // Resolviendo fallbacks
            const name = t.name || t.fullName || t.nombre || "Sin nombre";
            const company = t.company || t.businessName || t.empresa || t.despacho || "Sin empresa";
            const email = t.email || "";
            const phone = t.phone || t.telefono || "";
                                ...
                                <div key={token.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-4 hover:border-zinc-700 transition-colors">
                                    <div className="flex-1 space-y-1 min-w-0">
                                        ...
                                        <p className="text-sm font-medium">
                                            {token.name || token.fullName || token.nombre || "Sin nombre"}{" "}
                                            <span className="text-zinc-500">({token.company || token.businessName || token.empresa || token.despacho || "Sin empresa"})</span>
                                        </p>
                                        <div className="text-xs text-zinc-400 flex items-center gap-2">
                                            <span>{token.email} • {token.phone || token.telefono || ""}</span>
                                            {(token.phone || token.telefono) && (
                                                <button onClick={() => window.open(`https://wa.me/${(token.phone || token.telefono || "").replace(/\D/g, '')}`, "_blank")} className="text-[#25D366] hover:scale-110 transition-transform" title="Contactar por WhatsApp">
