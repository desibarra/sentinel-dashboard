...
export function exportToExcel(results: ValidationResult[], fileNameOverride?: string) {
  ...
  validResults.forEach(r => {
    uuidMap.set(r.uuid, {
      ...
      isPrincipal: true,
      tipoCFDI: r.tipoCFDI,
      tieneCPPrincipal: getCartaPortePresente(r) === 'SI',
      versionCP: cp(r)?.version || r.versionCartaPorte || 'NO_DETECTADA',
      isRelated: false,
      parentRefs: []
    });
  });
    ...
    if (r.uuids_relacionados && r.uuids_relacionados.length > 0) {
      r.uuids_relacionados.forEach(relUuid => {
        ...
        relNode.parentRefs.push({
          ...
          parentTieneCP: getCartaPortePresente(r) === 'SI',
          parentVersionCP: cp(r)?.version || r.versionCartaPorte || 'NO_DETECTADA',
