...
export interface CartaPorteUbicacionDetalle {
    idUbicacion: string;
    rfc: string;
    nombre: string;
    fechaHora: string;
...
export interface CartaPorteFiguraDetalle {
    tipoFigura: string;
    rfcFigura: string;
    nombreFigura: string;
    numLicencia: string;
...
export interface CartaPorteDetalle {
    ...
    origen: CartaPorteUbicacionDetalle;
    destino: CartaPorteUbicacionDetalle;
    totalMercancias: string;
    ...
    remolques: Array<{ subTipoRemolque: string; placaRemolque: string }>;
    figuras: CartaPorteFiguraDetalle[];
    operador: CartaPorteFiguraDetalle;
}
...
const NO_XML = "NO VIENE EN XML";

const emptyUbicacion = (): CartaPorteUbicacionDetalle => ({
    idUbicacion: NO_XML,
    rfc: NO_XML,
    nombre: NO_XML,
    fechaHora: NO_XML,
    calle: NO_XML,
    numeroExterior: NO_XML,
    numeroInterior: NO_XML,
    colonia: NO_XML,
    localidad: NO_XML,
    municipio: NO_XML,
    estado: NO_XML,
...
const emptyFigura = (): CartaPorteFiguraDetalle => ({
    tipoFigura: NO_XML,
    rfcFigura: NO_XML,
    nombreFigura: NO_XML,
    numLicencia: NO_XML,
    residenciaFiscal: NO_XML,
    numRegIdTrib: NO_XML,
});

const emptyCartaPorteDetalle = (): CartaPorteDetalle => ({
    presente: "NO",
    version: "NO APLICA",
    transporteInternacional: NO_XML,
    entradaSalidaMercancia: NO_XML,
    paisOrigenDestino: NO_XML,
    viaEntradaSalida: NO_XML,
    totalDistanciaRecorrida: NO_XML,
    origen: emptyUbicacion(),
    destino: emptyUbicacion(),
    totalMercancias: "0",
    pesoBrutoTotal: NO_XML,
    unidadPeso: NO_XML,
    numTotalMercancias: "0",
    mercancias: [],
    mercanciaPrincipal: NO_XML,
    autotransporte: {
        permisoSCT: NO_XML,
        numeroPermisoSCT: NO_XML,
        configuracionVehicular: NO_XML,
        placaVM: NO_XML,
        anioModeloVM: NO_XML,
        aseguradoraRC: NO_XML,
        polizaRC: NO_XML,
    },
    remolques: [],
    figuras: [],
    operador: emptyFigura(),
});

const nodeLocalName = (node: Element) => (node.localName || node.nodeName || "").split(":").pop() || "";

const attr = (node: Element | undefined | null, ...names: string[]) => {
    if (!node) return NO_XML;
...
const elementsByLocalName = (root: Element | Document | undefined | null, localName: string): Element[] => {
    if (!root) return [];
    return Array.from(root.getElementsByTagName("*")).filter((node: Element) => nodeLocalName(node) === localName);
};

const firstByLocalName = (root: Element | Document | undefined | null, localName: string) =>
    elementsByLocalName(root, localName)[0];

const isPresent = (value: string | undefined | null) =>
    Boolean(value && value !== NO_XML && value !== "NO APLICA" && value !== "No" && value !== "NO");

const extractDomicilio = (ubicacion: Element | undefined): Omit<CartaPorteUbicacionDetalle, "idUbicacion" | "rfc" | "nombre" | "fechaHora"> => {
    const domicilio = firstByLocalName(ubicacion, "Domicilio");
    return {
        calle: attr(domicilio, "Calle"),
        numeroExterior: attr(domicilio, "NumeroExterior", "NumExterior"),
        numeroInterior: attr(domicilio, "NumeroInterior", "NumInterior"),
        colonia: attr(domicilio, "Colonia"),
        localidad: attr(domicilio, "Localidad"),
        municipio: attr(domicilio, "Municipio"),
        estado: attr(domicilio, "Estado"),
        pais: attr(domicilio, "Pais"),
        codigoPostal: attr(domicilio, "CodigoPostal", "CP"),
        referencia: attr(domicilio, "Referencia"),
    };
};

export const extractCartaPorteDetalle = (xmlDoc: XMLDocument): CartaPorteDetalle => {
    const cartaPorte = elementsByLocalName(xmlDoc, "CartaPorte").find(node => {
        const ns = node.namespaceURI || "";
        return ns === "" || /CartaPorte/i.test(ns) || /cartaporte/i.test(node.nodeName);
    });

    if (!cartaPorte) return emptyCartaPorteDetalle();

    const ubicaciones = elementsByLocalName(cartaPorte, "Ubicacion");
    const origenNode = ubicaciones.find(node => attr(node, "TipoUbicacion") === "Origen");
    const destinoNode = ubicaciones.find(node => attr(node, "TipoUbicacion") === "Destino");
    const mercanciasNode = firstByLocalName(cartaPorte, "Mercancias");
    const autotransporteNode = firstByLocalName(cartaPorte, "Autotransporte");
    const identificacionVehicular = firstByLocalName(autotransporteNode, "IdentificacionVehicular");
    const seguros = firstByLocalName(autotransporteNode, "Seguros");
    const remolques = elementsByLocalName(autotransporteNode, "Remolque").map(node => ({
        subTipoRemolque: attr(node, "SubTipoRem", "SubTipoRemolque"),
        placaRemolque: attr(node, "Placa"),
    }));
    const figuras = elementsByLocalName(cartaPorte, "TiposFigura").map(node => ({
        tipoFigura: attr(node, "TipoFigura"),
        rfcFigura: attr(node, "RFCFigura", "RFC"),
        nombreFigura: attr(node, "NombreFigura", "Nombre"),
        numLicencia: attr(node, "NumLicencia"),
        residenciaFiscal: attr(node, "ResidenciaFiscalFigura", "ResidenciaFiscal"),
        numRegIdTrib: attr(node, "NumRegIdTribFigura", "NumRegIdTrib"),
    }));
    const operador = figuras.find(fig => fig.tipoFigura === "01" || isPresent(fig.numLicencia) || isPresent(fig.rfcFigura)) || emptyFigura();
    const mercancias = elementsByLocalName(mercanciasNode, "Mercancia").map(node => ({
        descripcion: attr(node, "Descripcion"),
        claveProdServCP: attr(node, "BienesTransp", "ClaveProdServ"),
        cantidad: attr(node, "Cantidad"),
        claveUnidad: attr(node, "ClaveUnidad"),
        pesoKg: attr(node, "PesoEnKg", "PesoEnKG"),
        valorMercancia: attr(node, "ValorMercancia"),
        moneda: attr(node, "Moneda"),
    }));

    const buildUbicacion = (node: Element | undefined): CartaPorteUbicacionDetalle => ({
        idUbicacion: attr(node, "IDUbicacion"),
        rfc: attr(node, "RFCRemitenteDestinatario"),
        nombre: attr(node, "NombreRemitenteDestinatario"),
        fechaHora: attr(node, "FechaHoraSalidaLlegada"),
        ...extractDomicilio(node),
    });

    return {
        presente: "SÍ",
        version: attr(cartaPorte, "Version"),
        transporteInternacional: attr(cartaPorte, "TranspInternac"),
        entradaSalidaMercancia: attr(cartaPorte, "EntradaSalidaMerc"),
        paisOrigenDestino: attr(cartaPorte, "PaisOrigenDestino"),
        viaEntradaSalida: attr(cartaPorte, "ViaEntradaSalida"),
        totalDistanciaRecorrida: attr(cartaPorte, "TotalDistRec"),
        origen: buildUbicacion(origenNode),
        destino: buildUbicacion(destinoNode),
        totalMercancias: attr(mercanciasNode, "NumTotalMercancias"),
        ...
        mercancias,
        mercanciaPrincipal: mercancias[0]?.descripcion || NO_XML,
        autotransporte: {
            permisoSCT: attr(autotransporteNode, "PermSCT"),
            numeroPermisoSCT: attr(autotransporteNode, "NumPermisoSCT"),
            configuracionVehicular: attr(identificacionVehicular, "ConfigVehicular"),
            placaVM: attr(identificacionVehicular, "PlacaVM", "Placa"),
            anioModeloVM: attr(identificacionVehicular, "AnioModeloVM", "Anio"),
            aseguradoraRC: attr(seguros, "AseguraRespCivil"),
            polizaRC: attr(seguros, "PolizaRespCivil"),
        },
        remolques,
        figuras,
        operador,
...
export const evaluarTrazabilidad = (xmlDoc: XMLDocument, xmlContent: string, r: any): TrazabilidadFiscalInfo => {
    ...
    const cartaPorteDetalle = extractCartaPorteDetalle(xmlDoc);
    ...
    let tieneCartaPorte = cartaPorteDetalle.presente === "SÍ" || r.cartaPorte === "SÍ" ? "SÍ" : "No";
    let tienePlacasUnidad = isPresent(cartaPorteDetalle.autotransporte.placaVM) ? "SÍ" : "No";
    let tieneRemolque = cartaPorteDetalle.remolques.length > 0 ? "SÍ" : "No";
    let tieneOperador = isPresent(cartaPorteDetalle.operador.rfcFigura) || isPresent(cartaPorteDetalle.operador.nombreFigura) || isPresent(cartaPorteDetalle.operador.numLicencia) ? "SÍ" : "No";
    let tieneOrigen = isPresent(cartaPorteDetalle.origen.idUbicacion) || isPresent(cartaPorteDetalle.origen.rfc) || isPresent(cartaPorteDetalle.origen.codigoPostal) ? "SÍ" : "No";
    let tieneDestino = isPresent(cartaPorteDetalle.destino.idUbicacion) || isPresent(cartaPorteDetalle.destino.rfc) || isPresent(cartaPorteDetalle.destino.codigoPostal) ? "SÍ" : "No";
    let tieneMercancias = cartaPorteDetalle.mercancias.length > 0 || isPresent(cartaPorteDetalle.pesoBrutoTotal) ? "SÍ" : "No";
    let tienePesoDistancia = isPresent(cartaPorteDetalle.pesoBrutoTotal) || isPresent(cartaPorteDetalle.totalDistanciaRecorrida) ? "SÍ" : "No";

    let placas = cartaPorteDetalle.autotransporte.placaVM;
    let remolques = cartaPorteDetalle.remolques.map(remolque => [remolque.subTipoRemolque, remolque.placaRemolque].filter(isPresent).join(" ")).filter(Boolean).join(" | ") || NO_XML;
    let rfcOperador = cartaPorteDetalle.operador.rfcFigura;
    let peso = [cartaPorteDetalle.pesoBrutoTotal, cartaPorteDetalle.unidadPeso].filter(isPresent).join(" ") || NO_XML;
    let distancia = cartaPorteDetalle.totalDistanciaRecorrida;
    let permisoSCT = cartaPorteDetalle.autotransporte.permisoSCT;
    let transporteInternacional = cartaPorteDetalle.transporteInternacional;
    ...
    if (r.estatusSAT === "Error Conexión" || r.estatusSAT?.includes("Error") || r.estatusSAT?.includes("timeout") || r.estatusSAT?.includes("CORS")) {
...
export const extractTaxesByConcepto = (xmlDoc: XMLDocument, version: string) => {
    ...
    if (conceptos) {
        for (let i = 0; i < conceptos.length; i++) {
            ...
            const objetoImp = nodo.getAttribute("ObjetoImp") || (version === "4.0" ? (impuestosConcepto ? "02" : "01") : "02");
