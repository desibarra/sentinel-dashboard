...
const getCartaPortePresente = (r: ValidationResult) => {
  ...
  return diagnostico === 'SÍ' || trazabilidad === 'SÍ' ? 'SÍ' : 'NO';
};

const getDatosFaltantes = (r: ValidationResult) => {
  const existing = r.trazabilidadInfo?.datosFaltantes;
  if (existing && existing !== 'Ninguno' && existing !== 'NO APLICA') return existing;

  const faltantes: string[] = [];
  if (getCartaPortePresente(r) === 'NO') faltantes.push('Carta Porte');
  if (normalizeSiNo(r.trazabilidadInfo?.tienePedimento) === 'NO') faltantes.push('Pedimento');
  if (normalizeSiNo(r.trazabilidadInfo?.tieneDoda) === 'NO') faltantes.push('DODA');
  if (normalizeSiNo(r.trazabilidadInfo?.tieneEntryNumber) === 'NO') faltantes.push('Entry');
  if (!String(r.trazabilidadInfo?.identificadorBancario || '').includes('SÍ')) faltantes.push('Identificación bancaria');
  ...
  return faltantes.length ? faltantes.join(' | ') : 'Sin faltantes críticos';
};
...
export function exportToExcel(results: ValidationResult[], fileNameOverride?: string) {
  ...
  const dataTasa0 = results.filter(r => (r.baseIVA0 || 0) > 0).map(r => ({
    ...
    Mercancias: normalizeSiNo(r.trazabilidadInfo?.tieneMercancias),
