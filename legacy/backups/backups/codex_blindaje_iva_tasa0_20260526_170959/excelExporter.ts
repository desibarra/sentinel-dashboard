...
const isTasa0Concept = (concepto: any) =>
  concepto?.traslados?.some((t: any) => t.impuesto === '002' && Number(t.base || 0) > 0 && (String(t.tasa) === '0' || String(t.tasa) === '0.00' || String(t.tasa) === '0.000000' || Number(t.tasa) === 0));

const getTasa0Base = (concepto: any) => {
  const traslados = concepto?.traslados?.filter((t: any) => t.impuesto === '002' && (String(t.tasa) === '0' || String(t.tasa) === '0.00' || String(t.tasa) === '0.000000' || Number(t.tasa) === 0)) || [];
  return traslados.reduce((sum: number, t: any) => sum + Number(t.base || 0), 0) || Number(concepto?.importe || 0) - Number(concepto?.descuento || 0);
};

const getTasa0Iva = (concepto: any) => {
  const traslados = concepto?.traslados?.filter((t: any) => t.impuesto === '002' && (String(t.tasa) === '0' || String(t.tasa) === '0.00' || String(t.tasa) === '0.000000' || Number(t.tasa) === 0)) || [];
  return traslados.reduce((sum: number, t: any) => sum + Number(t.importe || 0), 0);
};
...
export function exportToExcel(results: ValidationResult[], fileNameOverride?: string) {
  ...
  const dataAuditoriaIvaTasa0 = results.flatMap(r => {
    const conceptosTasa0 = (r.desglosePorConcepto || []).filter((concepto: any) => isTasa0Concept(concepto));
