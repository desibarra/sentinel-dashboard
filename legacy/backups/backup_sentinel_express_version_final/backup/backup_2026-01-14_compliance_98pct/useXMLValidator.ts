...
export interface ValidationResult {
  ...
  fechaEmision: string;
  horaEmision: string;
  añoFiscal: number; // ✅ SKILL v1.0.0 - REPORTES: Año fiscal para contexto temporal
  estatusSAT: string;
  ...
  cpReceptor: string;
  // ✅ SKILL v1.0.0 - CLASIFICACIÓN DOCUMENTAL: CfdiRelacionados
  tieneCfdiRelacionados: string; // "SÍ" | "NO"
  tipoRelacion: string; // "01"-"09" o "NO APLICA"
  uuidRelacionado: string; // UUID del CFDI relacionado o "NO APLICA"
  // ✅ SKILL v1.0.0 - CLASIFICACIÓN DOCUMENTAL: Tipo Real de Documento
  tipoRealDocumento: string; // "Factura" | "Nota de Crédito" | "Nota de Cargo" | "Pago (REP)" | "Nómina" | "Traslado" | "Egreso" | "Desconocido"
  requiereCartaPorte: string;
  cartaPorte: string;
  cartaPorteCompleta: string;
  versionCartaPorte: string;
  // ✅ SKILL v1.0.0 - COMPLEMENTO PAGOS: Validación temporal 1.0/2.0
  pagosPresente: string; // "SÍ" | "NO" | "NO APLICA"
  versionPagos: string; // "1.0" | "2.0" | "NO DISPONIBLE" | "NO APLICA"
  pagosValido: string; // "SÍ" | "NO" | "NO APLICA"
  // ✅ SKILL v1.0.0 - MULTI-ENCODING: Soporte UTF-8, ISO-8859-1, Windows-1252
  encodingDetectado: string; // "UTF-8" | "ISO-8859-1" | "Windows-1252" | "NO DECLARADO"
  // ✅ SKILL v1.0.0 - BLOQUE 8 - REPORTES COMPLETOS
  complementosDetectados: string[]; // ["Pagos 2.0", "Nómina 1.2", "CartaPorte 3.1"]
  scoreInformativo: number; // 0-100, NO bloqueante, solo informativo
  subtotal: number;
  baseIVA16: number;
  baseIVA8: number;
  baseIVA0: number;
  ...
  desglose: string; // Texto explicativo del desglose
  // CAMPOS DE NÓMINA 1.2
  esNomina: string;
...
export function useXMLValidator() {
  ...
  const validateSingleXML = async (
    ...
    try {
      ...
      const scoreInformativo = calcularScoreInformativo(
        resultado,
      ...
      return {
        ...
        total: totalXML,
        moneda,
        tipoCambio: moneda === "MXN" ? 1 : tipoCambio,
        formaPago,
        metodoPago,
        nivelValidacion,
        resultado,
